...
- Pretendard 폰트와 WebView 모바일 UI 기준을 따른 Native UI
...
Native 화면은 WebView 모바일 화면과 시각 기준을 맞춥니다. 상단 뒤로가기 버튼은 웹 `MIcon.back`과 같은 chevron path를 사용하고, 증명서 발급/상세/제출, QR 스캔, PIN/지문 인증, 복구 문구 백업, 지갑 복구 화면에 동일한 아이콘 규칙을 적용합니다.
